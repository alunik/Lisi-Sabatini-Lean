...
theorem mem_kSubgroup_of_piPrime_mem_centralizer [Finite G] (hG : IsMinimalSimple G)
...
theorem lt_top_of_mem_hInvariantStar [Finite G] (hG : IsMinimalSimple G)
...
theorem commonConstruction [Finite G] (hG : IsMinimalSimple G)
...
theorem inductiveLemma [Finite G] (hG : IsMinimalSimple G)
    {A : Subgroup G} (hA : Hypothesis71 A) {q : ℕ} [Fact q.Prime] (hq : q ∈ (primesOf A)ᶜ)
    {Q₁ Q₂ : Subgroup G} (hQ₁ : Q₁ ∈ hInvariantStar ⊤ A {q})
    (hQ₂ : Q₂ ∈ hInvariantStar ⊤ A {q})
    (H : Subgroup G) (hHproper : H < ⊤) (hAH : A ≤ H)
...
theorem centralizer_singleton_lt_top [Finite G] (hG : IsMinimalSimple G) {x : G}
...
theorem transitive_of_three_le_rank_center [Finite G] (hG : IsMinimalSimple G)
    {A : Subgroup G} (hA : Hypothesis71 A) {q : ℕ} [Fact q.Prime] (hq : q ∈ (primesOf A)ᶜ)
...
theorem hInvariantStar_eq_of_three_le_rank_center_of_kSubgroup_eq_bot [Finite G]
    (hG : IsMinimalSimple G) {A : Subgroup G} (hA : Hypothesis71 A)
...
theorem transitive_of_two_le_rank_center_of_dvd [Finite G] (hG : IsMinimalSimple G)
    {A : Subgroup G} (hA : Hypothesis71 A) {q : ℕ} [Fact q.Prime] (hq : q ∈ (primesOf A)ᶜ)
