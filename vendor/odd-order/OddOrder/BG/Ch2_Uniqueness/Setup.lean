...
`Odd (Nat.card G) → Group.IsSolvable G` is obtained by deriving `False` from `IsMinimalSimpleOdd G`
...
/-- The order-independent minimal-simple context used by local arguments that do not need
oddness: `G` is simple and nonsolvable, while every proper subgroup is solvable. -/
structure IsMinimalSimple (G : Type*) [Group G] [Finite G] : Prop where
  /-- `G` is simple. -/
  simple : IsSimpleGroup G
  /-- `G` is not solvable. -/
  notSolvable : ¬ Group.IsSolvable G
  /-- Every proper subgroup of `G` is solvable. -/
  properSolvable : ∀ M : Subgroup G, M < ⊤ → Group.IsSolvable M

namespace IsMinimalSimple

variable {G : Type*} [Group G] [Finite G]

/-- Every proper subgroup in a minimal-simple context is solvable. -/
theorem isSolvable_of_lt_top (hG : IsMinimalSimple G) (M : Subgroup G) (hM : M < ⊤) :
    Group.IsSolvable M :=
...
/-- Forget the odd-order field of a minimal odd-order counterexample. -/
theorem IsMinimalSimpleOdd.toIsMinimalSimple {G : Type*} [Group G] [Finite G]
    (hG : IsMinimalSimpleOdd G) : IsMinimalSimple G where
  simple := hG.simple
  notSolvable := hG.notSolvable
  properSolvable := hG.properSolvable

instance {G : Type*} [Group G] [Finite G] :
    Coe (IsMinimalSimpleOdd G) (IsMinimalSimple G) :=
  ⟨IsMinimalSimpleOdd.toIsMinimalSimple⟩
...
theorem isSolvable_of_lt_top (hG : IsMinimalSimpleOdd G) (M : Subgroup G) (hM : M < ⊤) :
    Group.IsSolvable M :=
  hG.properSolvable M hM

/-- Every maximal subgroup of a minimal counterexample is solvable. -/
theorem isSolvable_of_isCoatom (hG : IsMinimalSimpleOdd G) {M : Subgroup G} (hM : IsCoatom M) :
    Group.IsSolvable M :=
  hG.isSolvable_of_lt_top M hM.lt_top
...
/-- The `ℳ`-membership form of `isSolvable_of_isCoatom`. -/
theorem isSolvable_of_mem_maximalSubgroups (hG : IsMinimalSimpleOdd G) {M : Subgroup G}
    (hM : M ∈ OddOrder.GroupTheory.maximalSubgroups G) :
    Group.IsSolvable M :=
  hG.isSolvable_of_isCoatom (OddOrder.GroupTheory.mem_maximalSubgroups.mp hM)
...
theorem ne_bot_of_isCoatom (hG : IsMinimalSimpleOdd G) {M : Subgroup G}
  ...
  classical
  intro hMbot
  have : Nontrivial G := hG.nontrivial
  ...
  have : Group.IsNilpotent G := IsPGroup.isNilpotent hPp
